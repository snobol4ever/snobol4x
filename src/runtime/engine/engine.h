/*======================================================================================
 * engine.h — scrip-cc Byrd Box MATCH_fn engine
 *
...
enum {
    ...
    T_FUNC    = 44,     /* zero-width: call func(userdata) at MATCH_fn time; succeed if returns non-NULL */
};
...
typedef struct {
    ...
    int start;     /* always 0 (anchored MATCH_fn) */
    int end;       /* cursor position at MATCH_fn end */
} MatchResult;

/* Capture callback: fired when a T_CAPTURE node succeeds.
 * cap_slot: opaque int (INDEX_fn into the caller's capture table).
