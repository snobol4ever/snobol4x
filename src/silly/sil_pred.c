...
SIL_result FUNCTN_fn(void)
{
    if (VARVUP_fn() == FAIL) return FAIL;
    {
        ...
        /* AEQL XPTR,UNDFCL,RETNUL,FAIL */
        if (deql(XPTR, UNDFCL)) { MOVD(XPTR, NULVCL); return OK; }
        return FAIL;
    }
}
...
SIL_result NEG_fn(void)
{
    ...
    /* argument failed → skip it and return null */
    CODSKP_fn(D_A(ONECL));
    MOVD(XPTR, NULVCL);
    return OK;
}
...
SIL_result CHAR_fn(void)
{
    ...
    if (ACOMPC(XCL, 0) <= 0) { SETAC(ERRTYP, ERR_NEGATIVE); return FAIL; }
    /* ACOMPC XCL,256,INTR30,INTR30 — must be < 256 */
    if (ACOMPC(XCL, 256) >= 0) { SETAC(ERRTYP, ERR_ILLEGAL_ARG); return FAIL; }
...
static SIL_result rpad_common(void)
{
    pred_push(SCL);
    /* Get string arg */
    if (VARVAL_fn() == FAIL) { pred_top--; return FAIL; }
    pred_push(XPTR);
    /* Get integer length */
    if (INTVAL_fn() == FAIL) { pred_top -= 2; return FAIL; }
    MOVD(ZPTR, XPTR);
    pred_push(ZPTR);
    /* Get pad character */
    if (VARVAL_fn() == FAIL) { pred_top -= 3; return FAIL; }
    MOVD(WPTR, XPTR);
    ZPTR = pred_pop(); XPTR = pred_pop(); SCL = pred_pop();

    /* ACOMP ZPTR,MLENCL,INTR8 */
    if (ACOMP(ZPTR, MLENCL) > 0) { SETAC(ERRTYP, 8); return FAIL; }
    /* ACOMP ZEROCL,ZPTR,LENERR — negative length */
    if (ACOMP(ZEROCL, ZPTR) > 0) { SETAC(ERRTYP, ERR_NEGATIVE); return FAIL; }
    ...
    /* GETLG YPTR,XSP */
    D_A(YPTR) = XSP.l;
    /* ACOMP YPTR,ZPTR,RTXPTR,RTXPTR — already long enough? */
    if (ACOMP(YPTR, ZPTR) >= 0) return OK;   /* return XPTR unchanged */
    ...
    /* PAD_fn(dir, out, subj, pad) */
    PAD_fn(D_A(SCL), &TSP, &XSP, &VSP);
