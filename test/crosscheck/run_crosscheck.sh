...
for _src in \
    ...
    gcc -O0 -g -c "$_src" -I"$RT/x86" -I"$RT" -I"$RT/x86" -I"$RT/mock" -I"$SCRIP_INC" -I"$BACKEND_C" -w -o "$_o"
    _rt_objs+=("$_o")
done
...
run_test() {
    ...
    if ! "$SCRIP" -trampoline "$sno" > "$c" 2>/dev/null; then
        echo -e "${RED}FAIL${RESET} $name  [scrip]"; FAIL=$((FAIL+1))
        [[ "$STOP_ON_FAIL" == "1" ]] && summary && exit 1; return 1
    fi
    if ! gcc -O0 -g "$c" "$RTLIB" \
        -I"$RT/x86" -I"$RT" -I"$RT/x86" -I"$RT/mock" -I"$SCRIP_INC" -I"$BACKEND_C" \
