...
for icn in "$RUNG_DIR"/rung30_builtins_misc_*.icn; do
  base="${icn%.icn}"; exp="$base.expected"; [ -f "$exp" ] || continue
  [ -f "$base.xfail" ] && { XFAIL=$((XFAIL+1)); echo "XFAIL: $(basename $icn)"; continue; }
  ${1:-/tmp/scrip} -jvm "$icn" -o /tmp/t30.j 2>/dev/null
  timeout 30 java -jar src/backend/jasmin.jar /tmp/t30.j -d /tmp/ 2>/dev/null
