...
RUNG_DIR="${CORPUS_REPO:-$(cd "$SCRIPT_DIR/../../.." && pwd)/corpus}/programs/icon/rung12_strrelop_size"
for icn in "$RUNG_DIR"/t*.icn; do
  ...
  timeout 30 java -jar src/backend/jvm/jasmin.jar /tmp/t12_x.j -d /tmp/ >/dev/null 2>&1
