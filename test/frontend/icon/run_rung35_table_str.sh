...
RUNG_DIR="${CORPUS_REPO:-$(cd "$SCRIPT_DIR/../../.." && pwd)/corpus}/programs/icon/rung35_table_str"
for icn in "$RUNG_DIR"/t*.icn; do
  ...
  timeout 30 java -jar src/backend/jvm/jasmin.jar /tmp/t35_table_.j -d /tmp/ >/dev/null 2>&1
